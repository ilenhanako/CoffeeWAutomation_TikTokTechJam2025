...
Use business_goal ONLY to interpret blockers:
- - If a login, sign-up, or account-related prompt is present AND the business_goal requires actions that normally need authentication.
- If an OS permission dialog appears (camera, microphone, storage) AND the business_goal implies those, 
  then return recovery="GRANT_PERMISSION" with a concrete suggestion.
- If ads/upsells block progress, return recovery="HANDLE_INTERRUPT" with a close/skip suggestion.
- Otherwise, recovery suggestions should focus on completing the current micro-step only.
